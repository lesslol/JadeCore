...
namespace Kezan
{
    ...
    Creature* GetHomie(Player* player, uint32 entry, float dist)
    {
        ...
        JadeCore::CreatureLastSearcher<CreatureEntryAndSummonerGuidCheck> searcher(player, target, checker);
        JadeCore::VisitNearbyGridObject(player, dist, searcher);
        return target;
    }
        ...
        struct creature_script_impl : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    if (!targets.empty())
                    {
                        Creature* target = JadeCore::Containers::SelectRandomContainerElement(targets);
        ...
        struct creature_script_impl : public VehicleAI
        {
            ...
            void PassengerBoarded(Unit* who, int8 seatId, bool apply)
            {
                if (apply)
                {
                    ...
                    {
                        player->RemoveAura(SPELL_KEYS_TO_THE_HOT_ROD);
                        /* FIX ME! const uint8 homiesMask = Kezan::GetHomiesStatus(player);
...
class npc_kezan_bilgewater_buccaneer : public CreatureScript
{
    ...
    private:
        ...
        struct creature_script_impl : public VehicleAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                /* FIX ME! me->SetCustomVisibility(CUSTOM_VISIBILITY_SEER | CUSTOM_VISIBILITY_CREATURE, summoner->GetGUID());
            ...
            void PassengerBoarded(Unit* who, int8 seatId, bool apply)
            {
                ...
                if (Player* player = who->ToPlayer())
                {
                    if (apply)
                    {
                        ...
                        // FIX ME! player->setVehicleEjectPos(-8253.189f, 1481.026f, 41.544f, 0.865f);
                    }
                    else
                    {
    ...
    void DamageTaken(Unit* /*done_by*/, uint32 &damage)
    {
        if (me->GetHealth() <= damage)
        {
            ...
            if (!defeated)
            {
                ...
                // FIX ME! me->m_Events.AddEvent(new DelayEventDoAction(me, ACTION_RESPAWN), me->m_Events.CalculateTime(5000));
            }
        }
    }
    ...
    private:
        ...
        bool OnQuestAccept(Player* player, Creature* /*creature*/, Quest const* quest)
        {
            if (quest->GetQuestId() == QUEST_A_BAZILLION_MACAROONS)
            {
                ...
                {
                    // FIX ME! caster->SetCustomVisibility(CUSTOM_VISIBILITY_SEER | CUSTOM_VISIBILITY_CREATURE, player->GetGUID());
    ...
    private:
        ...
        struct creature_script_impl : public VehicleAI
        {
            ...
            void InitializeAI()
            {
                ...
                me->SetControlled(true, UNIT_STATE_ROOT);
                // FIX ME! me->setRegeneratingMana(false);
