...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    // MoP script - Norushen, handles corruption levels
    if (this && this->GetTypeId() == TypeID::TYPEID_PLAYER && victim && damage && damage != 0 && this->HasAura(144421))
    {
        if (AuraPtr l_Aura = this->GetAura(144421))
        {
            int32 bp = l_Aura->GetEffect(0)->GetBaseAmount();

            if (bp > 0)
            {
                float i = float((bp / 100));
    ...
    /*
    // Spark Of Zandalar
    if (GetTypeId() == TYPEID_PLAYER && damage != 0 && HasAura(138957))
    {
    ...
    */
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        if (ToPlayer()->GetSpecializationId(ToPlayer()->GetActiveSpec() == SPEC_MAGE_FIRE))
        {
            if (spellProto)
                if (spellProto->Id == 133 || spellProto->Id == 44614 || spellProto->Id == 108853 || spellProto->Id == 2948 ||
                    spellProto->Id == 11366 || spellProto->Id == 2120) // fire spec mastery ignite
                {
                    if (victim)
                    {
                        float value = GetFloatValue(PLAYER_MASTERY) / 100.0f;
                        int damage_X = damage * (value + 0.12);
                        CastCustomSpell(victim, 12654, &damage_X, NULL, NULL, true);
                    }
                }
        }
    }
    ...
    // Shadow infusion
    if (this && this->IsInWorld() && this->GetEntry() == 26125 && damage && victim)
    {
        if (this->HasAura(91342))
        {
            switch (this->GetAura(91342)->GetStackAmount())
            {
            case 1:
                damage *= 0.10;
                break;
    ...
    // convert fix below 50 remove aura
    if (victim->GetTypeId() == TYPEID_PLAYER)
    {
        if (victim->HasAura(122740))
        {
            if (!victim->HealthAbovePct(50))
            {
                victim->RemoveAura(122740);
            }
        }
    }
        ...
        case SPELLFAMILY_GENERIC:
        {
            ...
            case 138957: // Spark of Zandalar
                ...
                if (roll_chance_i(50))
                {
                    ...
                    {
                        AuraPtr aura = this->GetAura(138958);
                        ...
                        {
                            aura->Remove();
                            this->AddAura(138960, this);
                        }
                    }
                }
                break;
                case 138757: // Rentakal Soul Charm
                if (!victim)
                    return false;
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 138972: // Ji Kun's Rising Winds
                    {
                        if (!procSpell)
                            return false;
                        ...
                        if (this->ToPlayer()->HasSpellCooldown(138973))
                            return false;

                        if (this->ToPlayer()->GetHealthPct() > 35)
                            return false;

                        this->ToPlayer()->AddSpellCooldown(138973, 0, time(NULL) + 20);
                        break;
                    }
                    case 138790: // Wushoolays-final-choice
                    {
                        if (!procSpell)
                            return false;
                        ...
                        AuraPtr aura = this->GetAura(138786);
...
void Unit::setDeathState(DeathState s)
{
    ...
    else if (s == JUST_RESPAWNED)
    {
        ...
        if (this->GetTypeId() == TYPEID_PLAYER)
        {
            if (this->ToPlayer()->InArena() || this->ToPlayer()->InBattleground())
            {
                this->ClearInCombat();
                this->CombatStop();
                getHostileRefManager().deleteReferences();
...
void Unit::SetPower(Powers power, int32 val, bool regen)
{
    ...
    if (Player* _player = ToPlayer())
       {
        if (power == POWER_BURNING_EMBERS && val < 0)
            {
            int32 burn_embers = _player->GetPower(POWER_BURNING_EMBERS);
            if (burn_embers + val < 0)
                 val = -1 * burn_embers;
            
                if (_player->HasAura(108683))
                 _player->RemoveAura(108683);
            }
        }
...
int32 Unit::GetCreatePowers(Powers power) const
{
    switch (power)
    {
        ...
        case POWER_ENERGY:
            return ((isPet() && GetEntry() == 416 || GetEntry() == 417 || GetEntry() == 1860 || GetEntry() == 1863 || GetEntry() == 17252 || GetEntry() == 11859) ? 200 : 100);
        case POWER_RUNIC_POWER:
            return 1000;
        case POWER_RUNES:
            return 0;
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, AuraPtr aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    // Hack Fix for Divine Purpose
    if (spellProto && spellProto->Id == 86172 && procSpell)
    {
        uint32 allowed_spells[7] = { 85222, 85673, 114163, 53600, 53385, 84963, 85256 };
        bool can_trigger = false;
        for (uint16 i = 0; i < 7; i++)
        {
            if (procSpell->Id == allowed_spells[i])
            {
                can_trigger = true;
                break;
            }
