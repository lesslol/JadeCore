...
Item::~Item()
{
    ...
    if (Player * plr = GetOwner())
        if (plr->RemoveItemByDelete(this))
            sLog->OutJadeCore("Item %u on player guid %u is in destructor, and pointer is still referenced in player's data ...", GetEntry(), plr->GetGUIDLow());
}
